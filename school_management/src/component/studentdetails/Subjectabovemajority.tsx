...
const SubjectBelowMajority: React.FC<{ students: Student[] }> = ({
  students,
}) => {
  ...
  const findSubjectsBelowAverage = () => {
    ...
    const subjectsWithMajorityBelowAverage = Object.keys(
      belowAverageCounts
    ).filter((subject) => belowAverageCounts[subject] >= majorityThreshold);
