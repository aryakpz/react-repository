...
const HighEachSubject: React.FC<{ students: Student[] }> = ({ students }) => {
    const [topScorers, setTopScorers] = useState<{ subject: string; studentNames: string[]; mark: number }[]>([]);
    ...
    const findTopScorers = () => {
        ...
        const subjectTopScorers: { [subject: string]: { maxMark: number; students: Set<string> } } = {};

        students.forEach(student => {
            student.marks.forEach(({ subject, mark }) => {
                if (!subjectTopScorers[subject]) {
                    subjectTopScorers[subject] = { maxMark: mark, students: new Set([student.name]) };
                } else {
