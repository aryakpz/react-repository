...
const AvgAboveAtLeastOne: React.FC<AboveAverageAtLeastOneSubjectProps> = ({
  students,
  selectedStudent,
}) => {
  const [percentageAboveAverage, setPercentageAboveAverage] = useState<
    number | null
  >(null);
  ...
  const calculatePercentageAboveAverage = () => {
    ...
    students.forEach((student) => {
      ...
      student.marks.forEach((mark) => {
        if (
          averageMarks[mark.subject] !== undefined &&
          mark.mark > averageMarks[mark.subject]
        ) {
          scoredAbove = true;
        }
      });
