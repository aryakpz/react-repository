...
type HighestPercentageSubjectProps = {
  ...
  setHighestPercentageSubjects: React.Dispatch<
    React.SetStateAction<{ subject: string; percentage: number }[]>
  >; // Prop for setting the highest percentage subjects
};

const HighestPercentageSubject: React.FC<HighestPercentageSubjectProps> = ({
  students,
  selectedMark,
  setHighestPercentageSubjects,
}) => {
  useEffect(() => {
    const calculateHighestPercentageSubjects = () => {
      const subjectCounts: {
        [subject: string]: { aboveCount: number; totalCount: number };
      } = {};
      ...
      const subjectPercentages = Object.entries(subjectCounts).map(
        ([subject, counts]) => ({
          subject,
      ...
      const subjectsWithHighestPercentage = subjectPercentages.filter(
