...
const LowestPercentageSubjects: React.FC<LowestPercentageSubjectProps> = ({ students, selectedStudent }) => {
    ...
    const calculateLowestPercentageSubjects = () => {
            ...
            student.marks.forEach(mark => {
                ...
                if (mark.subject in subjectPercentages) {
                    ...
                    if (mark.mark < averageMarks) {
                        subjectPercentages[mark.subject] = (subjectPercentages[mark.subject] || 0) + 1;
                    }
                } else {
        ...
        const lowestPercentage = Math.max(...subjectPercentagesResult.map(sub => sub.percentage));
        const subjectsWithLowestPercentage = subjectPercentagesResult.filter(({ percentage }) => percentage === lowestPercentage);
    ...
    return (
        <p>
            <span>Subjects Below  Highest Percentage{selectedStudent}</span>
...
export default LowestPercentageSubjects;
