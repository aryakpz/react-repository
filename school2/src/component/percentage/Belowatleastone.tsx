...
const BelowAtleastOne: React.FC<{ students: Student[] }> = ({ students }) => {
    ...
    const calculatePercentage = () => {
        ...
        students.forEach(student => {
            const scoredBelowInOne = student.marks.some(mark => mark.mark < subjectAverages[mark.subject]);
            if (scoredBelowInOne) {
                studentsBelowAverageInAtLeastOne++;
            }
        });
 const percentageBelowInOne = (studentsBelowAverageInAtLeastOne / studentCount) * 100;
