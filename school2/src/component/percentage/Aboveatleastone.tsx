...
const AboveAtleastOne: React.FC<{ students: Student[] }> = ({ students }) => {
    ...
    const calculatePercentage = () => {
        ...
        students.forEach(student => {
            const scoredAboveInOne = student.marks.some(mark => mark.mark > subjectAverages[mark.subject]);
            if (scoredAboveInOne) {
                studentsAboveAverageInAtLeastOne++;
            }
        });

        const percentageAboveInOne = (studentsAboveAverageInAtLeastOne / studentCount) * 100;
