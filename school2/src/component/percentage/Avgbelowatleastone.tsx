...
const AvgBelowAtleastOne: React.FC<PercentageBelowAverageInAnySubjectProps> = ({ students, selectedStudent }) => {
    ...
    const calculatePercentageBelowAverage = () => {
        ...
        students.forEach(student => {
            if (student.name !== selectedStudent) { // Exclude the specific student
                const scoredBelowInAnySubject = student.marks.some(mark => mark.mark < averageMarks);
        ...
        const percentage = (studentsBelowAverageCount / (students.length - 1)) * 100; // Exclude the specific student from the total count
