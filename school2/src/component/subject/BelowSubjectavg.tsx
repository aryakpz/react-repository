        ...
        const calculateSubjectAverages = () => {
            ...
            students.forEach(student =>
                student.marks.forEach(mark => {
                    ...
                    if (mark.mark < avg) {
                        subjectsBelowAvg[mark.subject] = (subjectsBelowAvg[mark.subject] || 0) + 1;
                    }
                })
            );

            const majoritySubjects = Object.keys(subjectsBelowAvg).filter(subject => subjectsBelowAvg[subject] > students.length / 2);
