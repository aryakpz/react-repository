...
const TopScorerDisplay: React.FC<TopScorerDisplayProps> = ({ students, subject }) => {
    const highestScorer = students.reduce((max, student) => {
    ...
    return (
        <p>
            <span>Highest score in {subject}:</span> {highestScorer.name} - {highestScorer.mark}</p>
    );
};
